...
# Calculate scaled variograms for each phenotype using package function
variogram_results <- list()

cat("=== CALCULATING VARIOGRAMS FOR ALL TRAITS ===\n")
...
cat("\nVariograms calculated for:", length(variogram_results), "traits\n")
...
# Create spatial plots for all available traits using package function
if (all(c("x", "y") %in% names(field_data))) {
  ...
  spatial_plots <- list()
  for (trait in phenotypes) {
    if (trait %in% names(trait_labels)) {
      plot_title <- trait_labels[[trait]]
      spatial_plots[[trait]] <- create_spatial_plot(field_data, !!sym(trait), plot_title, trait)
    }
  }
  ...
  for (i in seq(1, length(plot_names), by = 3)) {
    ...
    for (j in seq_along(group_names)) {
      ...
      if (j == 1) {
        group_plots[[j]] <- spatial_plots[[plot_name]]
      } else {
    ...
    if (length(group_plots) > 1) {
      group_arranged <- do.call(ggarrange, c(group_plots, list(nrow = 1)))
